<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-gray-800">Sewa Lapangan Futsal</h2>
        </div>
    </x-slot>
    ...
    <div class="py-10 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="p-6 space-y-3">
                                    ...
                                    <ul class="text-sm text-gray-700">
                                        @foreach($field->prices as $price)
                                            <li>{{ $price->jam_mulai }} - {{ $price->jam_selesai }}: <span class="font-medium text-green-600">Rp {{ number_format($price->harga) }}</span></li>
                                        @endforeach
                                    </ul>
                ...
                @if ($fields->isEmpty())
                    <p class="text-gray-600">Belum ada lapangan futsal tersedia.</p>
                @endif
            </div>
        </div>
    </div>
